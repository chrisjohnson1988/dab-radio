...
class FicObserver {
public:
	virtual void ficData(uint8_t* data, int len) {}
};

class MscObserver {
public:
	virtual void mscData(const std::vector<uint8_t>& data) {}
};

class RaonTunerInput {

public:
    explicit RaonTunerInput();
    virtual ~RaonTunerInput();

    void initialize();
    bool isInitialized() const;
    int getCurrentTunedFrequency() const;
    void tuneFrequency(uint32_t frequencyKHz);
    void openSubChannel(uint8_t subchanId);
    void closeSubchannel(uint8_t subchanId);
    void readData();

    void setFicObserver(FicObserver* fic_observer) { m_fic_observer = fic_observer; };
    void setMscObserver(MscObserver* msc_observer) { m_msc_observer = msc_observer; };
...
private:
    ...
    FicObserver* m_fic_observer{nullptr};
    MscObserver* m_msc_observer{nullptr};
    
    uint32_t m_currentFrequency{0};

    libusb_context* m_usbCtx{nullptr};
    libusb_device_handle* m_usbDevice{nullptr};
...
private:
    ...
    int bulkTransferData(uint8_t endPointAddress, std::vector<uint8_t>& buffer);
};
